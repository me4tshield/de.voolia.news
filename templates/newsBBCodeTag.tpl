<div class="container containerPadding marginTop">
	...
	<div>
		<div class="containerHeadline">
			...
			<p><small><span class="username">{if $news->userID}<a href="{link controller='User' object=$news->getUserProfile()}{/link}" class="userLink" data-user-id="{$news->userID}">{$news->username}</a>{else}{$news->username}{/if}</span> - {@$news->time|time} - {lang}news.entry.bbcode.comments{/lang}</small></p>
		</div>
	</div>
