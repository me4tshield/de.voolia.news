{if $__wcf->getSession()->getPermission('user.news.canViewNews') && $newsList|count}
	...
	<div class="container containerPadding marginTop dashboardContentBoxHotNews">
				...
				<li class="newsSlideshow">
					...
					<div>
						<div class="containerHeadline">
							...
							<p><small><span class="username">{if $news->userID}<a href="{link controller='User' object=$news->getUserProfile()}{/link}" class="userLink" data-user-id="{$news->userID}">{$news->username}</a>{else}{$news->username}{/if}</span> - {@$news->time|time}{if $news->isCommentable()} - {lang}news.dashboard.box.hotnews.comments{/lang}{/if}</small></p>
						</div>
					</div>
